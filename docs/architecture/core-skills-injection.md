...
> ⚠️ **已过时**  
> 本文档描述的自动注入机制已于 2026-04-22 废除。  
> 现在 Skills 完全由 Agent 配置文件决定。  
> 请参阅: [Skills 注入机制重大变更](./skills-injection-change.md)

---

## 概述（历史记录）

系统**曾经**设计了一套**核心 Skills 强制注入机制**，确保所有 Agent（包括子 Agent）都具备基础能力。

**当前状态**: 此机制已被移除，Skills 现在完全由 Agent 配置文件的 `skills` 数组决定。

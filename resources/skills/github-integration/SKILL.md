...
```
获取 PR coobee/coobee-agent#123 的详情
```

**内部实现**（Agent 自动调用）：

```python
python3 ~/.coobee-agent/skills/github-integration/scripts/pr_info.py \
  --owner coobee \
  --repo coobee-agent \
  --number 123
```
...
```
列出 PR coobee/coobee-agent#123 修改的文件
```

**内部实现**：

```python
python3 ~/.coobee-agent/skills/github-integration/scripts/pr_files.py \
  --owner coobee \
  --repo coobee-agent \
  --number 123
```
...
```
在 PR coobee/coobee-agent#123 上评论："代码看起来不错，但建议增加单元测试"
```

**内部实现**：

```python
python3 ~/.coobee-agent/skills/github-integration/scripts/pr_comment.py \
  --owner coobee \
  --repo coobee-agent \
  --number 123 \
...
```
批准 PR coobee/coobee-agent#123，评论："LGTM，代码质量良好"
```

**内部实现**：

```python
python3 ~/.coobee-agent/skills/github-integration/scripts/pr_review.py \
  --owner coobee \
  --repo coobee-agent \
  --number 123 \
  --event APPROVE \
...
```
检查 PR coobee/coobee-agent#123 的 CI 状态
```

**内部实现**：

```python
python3 ~/.coobee-agent/skills/github-integration/scripts/check_runs.py \
  --owner coobee \
  --repo coobee-agent \
  --ref <commit_sha>
```
...
coobee-agent 提供 GitHub Webhook 端点：`POST /webhooks/github`
...
```
帮我审查 PR coobeea/coobee-agent#456，重点关注安全性和性能
```

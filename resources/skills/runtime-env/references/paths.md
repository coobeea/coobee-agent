...
| 键           | 说明           | 示例                       |
| ------------ | -------------- | -------------------------- |
| `userHome`   | 应用主目录     | ~/.coobee-agent               |
| `systemHome` | 系统用户目录   | /Users/xxx 或 C:\Users\xxx |
| `configDir`  | 配置目录       | ~/.coobee-agent/config        |
| `agentsDir`  | Agent 定义目录 | ~/.coobee-agent/agents        |
| `threadsDir` | 会话线程目录   | ~/.coobee-agent/threads       |
| `memoryDir`  | 记忆目录       | ~/.coobee-agent/memory        |
...
// ❌ 错误
const configPath = '~/.coobee-agent/config/coobee.json5';
```

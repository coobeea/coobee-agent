...
export class WindowManager implements IWindowManager {
  ...
  async createTab(windowId: number, config: TabConfig): Promise<number | null> {
    ...
    if (windowInfo.tabs.size >= this.MAX_TABS_PER_WINDOW) {
      log.warn(`[WindowManager] 窗口 Tab 数量已达上限: windowId=${windowId}, count=${windowInfo.tabs.size}`);
      return null;
    }
    ...
    try {
      ...
      log.info(`[WindowManager] Tab 创建成功: tabId=${tabId}, windowId=${windowId}, title=${tabInfo.title}`);
  ...
  async switchTab(windowId: number, tabId: number): Promise<boolean> {
    ...
    try {
      ...
      log.info(`[WindowManager] Tab 切换成功: windowId=${windowId}, tabId=${tabId}, title=${tabInfo.title}`);
  ...
  async closeTab(windowId: number, tabId: number): Promise<boolean> {
    ...
    try {
      ...
      if (remainingTabsCount === 0) {
        ...
        // 还有其他 Tab
        log.info(`[WindowManager] Tab 关闭成功: tabId=${tabId}, title=${tabTitle}, 剩余=${remainingTabsCount}`);
        ...
        let position = 0;
        for (const tab of Array.from(windowInfo.tabs.values()).sort((a, b) => a.position - b.position)) {
  ...
  async moveTabToWindow(tabId: number, fromWindowId: number, toWindowId: number): Promise<boolean> {
    ...
    if (toWindow.tabs.size >= this.MAX_TABS_PER_WINDOW) {
      log.warn(`[WindowManager] 目标窗口 Tab 数量已达上限: windowId=${toWindowId}, count=${toWindow.tabs.size}`);
      return false;
    }
  ...
  private async createDefaultTab(windowId: number, windowType: WindowType): Promise<void> {
    try {
      ...
      const defaultTabConfig: TabConfig = {
        url: windowType === 'agent' ? 'local://' : 'about:blank',
        title: windowType === 'agent' ? 'Coobee Agent' : 'New Tab',
        active: true,
        closable: true // 都可以关闭
      };
  ...
  createWindow(config: WindowConfig): BrowserWindow | null {
    ...
    try {
      ...
      log.info(`[WindowManager] 窗口创建成功: windowId=${window.id}, type=${config.type}, isMain=${windowInfo.isMain}`);
  ...
  private printWindowsState(action: string): void {
    const totalTabs = Array.from(this.windows.keys()).reduce((sum, wid) => sum + this.getWindowTabs(wid).length, 0);
    log.debug(`[WindowManager] ${action} | windows=${this.windows.size}, tabs=${totalTabs}`);
  }
  ...
  private updateViewBounds(window: BrowserWindow, view: WebContentsView): void {
    try {
      ...
      log.debug(`[WindowManager] 更新 View 边界: width=${bounds.width}, height=${bounds.height - CHROME_HEIGHT}`);
    } catch (error) {
  ...
  private setupTabEvents(webContents: Electron.WebContents, tabInfo: TabInfo): void {
    ...
    if (Env.isDev) {
      webContents.on(WebContentsEvents.CONSOLE_MESSAGE, (_event, level, message, _line, _source) => {

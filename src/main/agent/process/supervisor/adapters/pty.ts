...
// 用运行期字符串做 specifier，避免 TS 静态检查 node-pty 是否已安装。
// node-pty 是可选依赖：只有启用 PTY 模式时才需要装 `@lydell/node-pty`。
const PTY_MODULE_NAME = '@lydell/node-pty';

let ptyModulePromise: Promise<PtyModule> | null = null;

async function loadPtyModule(): Promise<PtyModule> {
  ptyModulePromise ??= import(/* @vite-ignore */ PTY_MODULE_NAME) as Promise<unknown> as Promise<PtyModule>;
...
export async function createPtyAdapter(params: {
  ...
  let resolveWait: ((value: { code: number | null; signal: NodeJS.Signals | number | null }) => void) | null = null;
  let waitPromise: Promise<{ code: number | null; signal: NodeJS.Signals | number | null }> | null = null;
  ...
  const settleWait = (value: { code: number | null; signal: NodeJS.Signals | number | null }): void => {
  ...
  const onStderr = (_listener: (chunk: string) => void): void => {
    // PTY 是一股合并的输出流，stderr 不单独发。
  };

  const wait = async (): Promise<{ code: number | null; signal: NodeJS.Signals | number | null }> => {
    if (waitResult) return waitResult;
    if (!waitPromise) {
      waitPromise = new Promise<{ code: number | null; signal: NodeJS.Signals | number | null }>((resolve) => {
        resolveWait = resolve;
        if (waitResult) {

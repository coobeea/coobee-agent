...
/**
 * Windows 控制台输出解码：很多系统工具（ipconfig / systeminfo 等）仍然按系统
 * code page 而非 UTF-8 发字节。拿到 stdout 字节后先尝试严格 UTF-8，失败再用
 * code page 对应的编码，流式场景还要处理跨 chunk 的半截字符。
 */
...
export function createWindowsOutputDecoder(params?: { platform?: NodeJS.Platform; windowsEncoding?: string | null }): {
  ...
  const legacyDecoder =
    platform === 'win32' && encoding && normalizedEncoding !== 'utf-8' ? new TextDecoder(encoding) : null;

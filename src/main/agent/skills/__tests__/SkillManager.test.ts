...
Content.`;
  ...
  describe('Skill 配置注入', () => {
    ...
    beforeEach(() => {
      tmpDir = path.join('/tmp', `skillmanager-config-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    it('配置齐全时 configStatus 为 configured', () => {
      createSkillWithConfig('ocr', 'paddle-ocr', '  - key: apiKey\n    description: Key\n    required: true\n');
      fs.writeFileSync(path.join(configDir, 'skills.json5'), JSON5.stringify({ 'paddle-ocr': { apiKey: 'sk-xxx' } }));

 ...
 *
 * 核心方法：
 *   forward(chunk) — 接收细粒度 AgentStreamChunk，自动映射为粗粒度 StreamMessage 并广播
 *
 * 设计原则：
 *   - Runtime 只 yield AgentStreamChunk（单一职责）
 *   - AgentExecutor 在消费 chunk 时调用 forward() 进行广播（统一出口）
 *   - AgentStreamChunk → StreamMessage 映射逻辑集中在此处
 */

import { eventBus } from '@main/common/eventbus';
import { SnowflakeIdGenerator } from '@main/utils';
import type { AgentStreamChunk, AgentStreamChunkType } from '../runtime/types';
...
export interface IStreamEmitter {
  /**
   * 转发 AgentStreamChunk 到 EventBus（核心方法）
   *
   * 直接透传 chunk.type，不做映射。前端不需要的事件忽略即可。
   * 同时触发生命周期事件（start/end/error）。
   */
  forward(chunk: AgentStreamChunk): void;
 ...
 */
const LIFECYCLE_EVENT_MAP: Partial<Record<AgentStreamChunkType, StreamEventType>> = {

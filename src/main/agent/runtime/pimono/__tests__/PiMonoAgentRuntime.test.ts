...
function logTestResult(
  ...
  if (opts.timedChunks && opts.timedChunks.length > 0) {
    try {
      ...
      for (const tc of opts.timedChunks) {
        appendEventsLog(`#${String(tc.seq).padStart(3)} [${String(tc.elapsed).padStart(6)}ms] ${tc.type}`);
...
function assertReasoningSeparation(chunks: StreamChunk[], output: string, testName: string): void {
  ...
  if (reasoningStarts > 0 || reasoningDones > 0) {
    ...
    expect(reasoningDeltas.length, `[${testName}] 有 reasoning:start 则必须有 reasoning:delta`).toBeGreaterThan(0);

    testLog(`${LOG_PREFIX}   [reasoning] ${reasoningStarts} 个推理块, ${reasoningDeltas.length} 个 delta`);
  }
}
...
describe.skipIf(!RUN)('PiMonoAgentRuntime 真实执行测试（OpenAI 兼容格式）', () => {
  ...
  beforeAll(() => {
    ...
    testLog(`${LOG_PREFIX} API: openai-completions, model=${apiConfig.model}, ` + `baseURL=${apiConfig.baseURL}`);
  ...
  it('场景1 - 简单问答：完整 run → turn → llm → text 闭环', { timeout: 60_000 }, async () => {
    ...
    assertOrdered(seq, 'run:start', 'turn:start', 'llm:start', 'text:delta', 'llm:done', 'turn:done', 'run:done');
  ...
  it('场景7 - Skill 和 AppendInstructions 注入：LLM 能使用注入的领域知识', { timeout: 60_000 }, async () => {

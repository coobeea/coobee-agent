...
/** 恢复动作 */
export type RecoveryAction = { action: 'retry'; delay?: number; reason: string } | { action: 'throw'; reason: string };
...
export interface RecoveryContext {
  ...
  runtime?: {
    ...
    setThinkingLevel?: (level: 'high' | 'medium' | 'low' | 'off') => void;
  };
}
  ...
  async recover(error: Error, context: RecoveryContext): Promise<RecoveryAction> {
    for (const strategy of this.strategies) {
      if (strategy.canHandle(error)) {
        log.info(`[ErrorRecovery] Strategy "${strategy.name}" matched for: ${error.message.slice(0, 100)}`);

...
function formatSessionItemsMarkdown(items: SessionItem[]): string {
  ...
  for (const si of items) {
    const role = (si.item as { role?: string }).role || (si.item as { type?: string }).type || 'unknown';
    let content = '';
    ...
    lines.push(`| ${si.seq} | ${si.type} | ${role} | ${content.replace(/\|/g, '\\|')}${metaInfo} | ${ts} |`);
  }
...
describe.skipIf(!RUN)('事件流 + Session 完整分析', () => {
  ...
  it('场景2：工具调用 — 完整事件流 + Session + Context', { timeout: 90_000 }, async () => {
    ...
    runtime = new OpenAIAgentRuntime({
      name: 'ToolAgent',
      instructions: '你是一个全能助手。加法用 add_numbers 工具，反转字符串用 reverse_string 工具。根据工具结果回答。',
      model: MODEL,
    ...
    if (rStarts > 0) {
      ...
      md.push(`- reasoning:start/delta/done: ${rStarts}/${ofType(chunks, 'reasoning:delta').length}/${rDones}`);
      md.push(
        `- text:delta 拼接: \`${ofType(chunks, 'text:delta')
          .map((c) => c.content)
          .join('')
  ...
  it('场景3：多轮对话 — Session 累积 + Context 构建验证', { timeout: 120_000 }, async () => {
    ...
    // 验证结论
    md.push(`### 3.7 验证结论\n`);
    md.push(`- Session 累积增长: ${sessionItems1.length} → ${sessionItems2.length} → ${sessionItems3.length} ✅`);
    md.push(
    ...
    summaryMd.push(`1. **事件闭环**: 所有 \`start/done\` 事件正确配对（run, turn, llm, text, reasoning, tool）`);
    summaryMd.push(

 ...
 * 前后端事件类型定义和 Payload。
 *
 * 目前主要用于 Electron IPC → renderer eventbus 的事件通道。
 */

import type { EventMessage } from './message';
...
export type IpcEventType = Extract<keyof EventPayloads, string>;

/**
 * IPC 事件消息格式。
 *
 * @deprecated 事件消息信封请优先使用 EventMessage；该类型保留为 IPC 强类型别名。
 */
export type IpcEventMessage<T extends IpcEventType = IpcEventType> = EventMessage<T, EventPayloads[T]>;

export type { EventMessage } from './message';

/**
 * 类型安全的事件处理器
 */
export type EventHandler<T extends IpcEventType> = (payload: EventPayloads[T]) => void;

...
<template>
  <aside class="flex flex-col w-60 shrink-0 bg-surface border-r border-border">
    ...
    <div class="flex flex-col flex-1 min-h-0 mt-3 border-t border-border">
      ...
      <div class="flex-1 overflow-y-auto px-3 pt-1.5 pb-3 flex flex-col gap-0.5 session-list">
        <!-- 任务列表 -->
        <div
          v-for="thread in recentThreads"
          :key="thread.id"
          class="session-item relative flex items-start px-3 py-2.5 rounded-lg text-sm cursor-pointer transition-all duration-200 ease-[cubic-bezier(0.4,0,0.2,1)]"
          :class="
          ...
          <div class="flex flex-col gap-0.5 min-w-0">
            <div class="flex items-center gap-1 min-w-0">
              <span
                class="overflow-hidden text-ellipsis whitespace-nowrap leading-snug text-sm font-medium flex-1 min-w-0 transition-colors duration-200"
                :class="{ 'font-semibold': activeThreadId === thread.id }">
                {{ thread.title }}
              </span>
              <div
                v-if="isThreadStreaming(thread.id)"
                class="streaming-wave flex items-center gap-0.5 h-3 ml-0.5 shrink-0"
                title="正在执行">
            ...
            <span
              class="text-xs leading-snug line-clamp-2 transition-colors duration-200"
              :class="activeThreadId === thread.id ? 'text-primary' : 'text-muted-foreground'">
        ...
        <!-- 空态 -->
        <div
          v-if="recentThreads.length === 0 && !threadsStore.loading"
          class="flex flex-col items-center gap-3 px-4 py-10 text-muted-foreground text-xs leading-relaxed text-center select-none">
          <span class="i-carbon-task-star inline-block h-6 w-6" />
          <p>选择智能体并开启任务后<br />任务将出现在这里</p>
        </div>

        <!-- 加载中 -->
        <div
          v-if="threadsStore.loading && recentThreads.length === 0"
          class="flex flex-col items-center gap-3 px-4 py-10 text-muted-foreground text-xs leading-relaxed text-center select-none">
